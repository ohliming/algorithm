...
def process(conn, report_date, data_file):
    ...
    val = ''
    for uid_info in j['daily_login_uids']:
        uid,is_investor,from_mobile,is_visit_cf = uid_info.strip().split('\t')
        val += "(%s,'%s',%s,%s,%s)," %(uid,report_date,is_investor,from_mobile,is_visit_cf)
    val = val.strip(',')
    sql_cmd = "REPLACE INTO user_login(uid,stat_date,is_investor,from_mobile,is_visit_cf) VALUES %s " %(val)
